...
\small{
	\caption{Hierarchization.}
 	...
 	\begin{algorithmic}[1]
 		\For{$t=1$ to $D$}
 			\For{$k=L$ downto $1$}
 				\State \Call{HierarchizeKernel}{$t, k$}
 			\EndFor
 		\EndFor
 		\algstore{bkbreak}
	\end{algorithmic}
	...
	\begin{algorithmic}[1]
  		\algrestore{bkbreak}
  		\Procedure{HierarchizeKernel}{$t, k$}
	 		\For{$j \leftarrow \Call{BlockStart}{k} + lane; j < \Call{BlockEnd}{k};	j\leftarrow j + 32$}
	 			\If{$lane = 0$}
...
$t$ variable over all dimensions, with $k$ variable over all refinement levels
...
refinement levels from line 2 inside the kernel. In this way, one warp will not
...
We move now to the algorithm for evaluation.
