...
indices in \textit{asg1d} of all $g-1$ parent \textit{blocks} before the
innermost loop. We store them in an array (in the heap space in the case of CPU
...
to index \textit{asg1d} and get the start of the \textit{block}. What we still
need is the index inside the \textit{block}. This is computed inside the $j$
loop by converting $\bar{i}$ to the corresponding index.
...
of storing the points horizontally, one point per row, it stores them
