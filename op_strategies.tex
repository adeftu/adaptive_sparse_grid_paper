...
index arrays which represent the coordinates of one point in the grid. We store
...
Regarding memory locality, we present two optimizations, \textit{ichg2'} and
\textit{ichg2''} which do a loop interchange by moving the loop over
interpolation points from line 2 inside the loop over \textit{blocks} from
line5. In addition, \textit{ichg2''} also moves the loops over \textit{groups}
...
them with multiplications with the inverse.
% 
% \begin{algorithm}[tbp]
% \small{
%  \caption{Evaluation.}
